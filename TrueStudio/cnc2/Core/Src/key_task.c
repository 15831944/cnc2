...
key_counter_t key_cnt[KEY_NUM] = {{KEY_CNT_MAX, TRUE}};
static keys_t keys, keys_flt, clicked;
...
keys_t getKeys() {
	keys_t keys;
	keys.data = (uint8_t)((GPIOG->IDR) >> 2) & KEY_MASK;
	return keys;
}
