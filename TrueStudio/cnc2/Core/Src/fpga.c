...
void fpga_snapMotor() { fpga_write_u16(MTR_CONTROL>>1, MTR_SNAPSHOT_BIT); }
...
void fpga_setTimeoutEnable(BOOL value) { fpga_write_u16(TIMEOUT_ENA>>1, 1); }
...
BOOL fpga_getCentered() {
	BOOL res = (fpga_getADCStatus() & ADC_CENTER_FLAG) != 0;
	fpga_clearCentered();
	return res;
}
...
void fpga_setCenterMode(FPGA_CENTER_MODE_T mode) { fpga_write_u16(CENTER>>1, (uint16_t)mode); }
uint16_t fpga_getCenterMode() { return fpga_read_u16(CENTER>>1); }
