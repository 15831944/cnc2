...
//    const uint32_t CENTER_ANGLE3    = 0x79 << 2;
    ...
    const uint32_t ABORT_MASK   = 1U << 1;
...
public:
    static constexpr float SPEED_MAX = 18; // mm/min
private:
    static CncCom m_com;
...
public:
    ...
    bool centering(
            CENTER_MODE_T mode,
            int touches, int attempts, int drumVel, int thld, int fineSharePct, float R, float rollback,
    ...
    cnc_context_t readBackup() const;
    void clearBackup();
    void initialContext(const cnc_context_t& ctx);

    cnc_adc_t readADC() const;
    cnc_adc_volt_t readADCVolt() const;

    double readDiameter() const;
